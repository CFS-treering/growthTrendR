...
if (rep_site %in% reports.sel) dt.site.out <- growthTrendR:::table_spc_site(dt.radii.spc, rw_ref, N.nbs.qa, scale.max_dist_km) else dt.site.out <- data.table()
...
for (page in seq(1, n_plots, by = plots_per_page)) {
  ...
  plots_with_margin <- lapply(plots_this_page, function(p) {
  ...
  # )
   gridExtra::grid.arrange(grobs = plots_with_margin, ncol = length(plots_with_margin))
}
